...
class Detector:
    def __init__(self):
        ...
        if USE_PYTORCH:
            ...
            self.model = torchvision.models.detection.ssd300_vgg16(weights=weights, box_score_thresh=0.5)
            self.model.eval()
        ...
        self.listObjectsInterest = [17, 18, 19, 22, 24] # DEBUG: see if we need to put back 13, it is related to stop sign
    ...
    def filter(self, boxes, scores, classes, num):
        ...
        for i in range(int(num)):
            if scores[i] >= MIN_SCORE:
                if self.listObjectsInterest.__contains__(int(classes[i])):
